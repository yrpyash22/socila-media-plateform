...
const Posts = (props) => {
  ...
  const clickLike = () => {
    ...
    callApi({userId : user1.id} , {t:jwt.token}, props.post._id).then((data)=>{
      if(data) {
    ...
    <div>
      <section className="posts border border_radius border-secondary mt-3 overflow-hidden p-0 white border position-relative">
        {props.post.userDetails?.id == user1.id ? (
          <i className="fa-regular fa-trash-can position-absolute pt-4 fs-5 trash" onClick={deletePost}/>
        ) : null}

        <div className="name d-flex pl-3 pt-3">
          <div>
            <img
              src={props.post.author?.image || "https://cdn-icons-png.flaticon.com/512/149/149071.png"}
              alt="profile"
          ...
          <div>
            <p onClick={() => { nav('/user/' + props.post.userDetails?.id) }} className="mb-0 fw-bold fs-5" style={{cursor: 'pointer'}}>
        ...
        <h5 className="mt-0 ml-1">
          <p className='fff'>
            ...
            {props.post.caption && props.post.caption.length > 300 ? (
              <span onClick={() => { setCaption(props.post.caption) }} className="badge text-primary" style={{cursor: 'pointer'}}>
                ...show more
              </span>
            ) : null}
          </p>
        ...
        {props.post.photo && (
          <div className="">
            <img src={props.post.photo} alt="post" className="w-100" />
          </div>
        )}
        ...
        <div className="d-flex justify-content-between p-3 align-items-center border-bottom ">
          <div className="d-flex ">
            <div className="d-flex me-5" style={{cursor: 'pointer'}}>
              {/* FIX: 'class' badal kar 'className' kiya gaya hai */}
              {!values.like ? (
                <i onClick={clickLike} className="fa-regular me-2 fs-2 fa-heart"></i>
              ) : (
